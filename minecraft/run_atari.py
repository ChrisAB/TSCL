...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--wrappers', choices=['deepmind', 'universe'], default='deepmind')
    # model
    parser.add_argument('--policy', choices=['cnn', 'rnn'], default='rnn')
    parser.add_argument(
        '--cnn_architecture', choices=['deepmind', 'alec', 'openai', 'homogeneous'], default='deepmind')
    ...
    parser.add_argument(
        '--rnn_type', choices=['lstm', 'gru', 'simple'], default='lstm')
    parser.add_argument('--fc_layers', type=int, default=0)
    parser.add_argument('--fc_size', type=int, default=32)
    parser.add_argument('--cnn_init', choices=['orthogonal', 'glorot_normal',
                                               'glorot_uniform', 'he_normal', 'he_uniform'], default='glorot_normal')
    parser.add_argument('--fc_init', choices=['orthogonal', 'glorot_normal',
                                              'glorot_uniform', 'he_normal', 'he_uniform'], default='glorot_normal')
    parser.add_argument(
        '--action_init', choices=['orthogonal', 'normal', 'uniform'], default='orthogonal')
    parser.add_argument('--action_init_scale', type=float, default=0.01)
    parser.add_argument(
        '--activation', choices=['tanh', 'relu', 'elu'], default='relu')
    parser.add_argument('--stochastic', '-s',
                        action='store_true', default=True)
    parser.add_argument('--deterministic', '-d',
                        action='store_false', dest='stochastic')
    # optimization
    parser.add_argument(
        '--optimizer', choices=['adam', 'rmsprop', 'nadam'], default='adam')
    ...
    parser.add_argument(
        '--policy_loss', choices=['pg', 'pposgd'], default='pposgd')
    ...
    parser.add_argument('--no_normalize_observations',
                        action='store_false', dest='normalize_observations')
    parser.add_argument('--normalize_advantage',
                        action='store_true', default=True)
    parser.add_argument('--no_normalize_advantage',
                        action='store_false', dest='normalize_advantage')
    parser.add_argument('--normalize_baseline',
                        action='store_true', default=True)
    parser.add_argument('--no_normalize_baseline',
                        action='store_false', dest='normalize_baseline')
    ...
    # parallelization
    parser.add_argument(
        '--trainer', choices=['batched', 'batched_predictor', 'batched_profiler'], default='batched')
    ...
    if args.command == 'train':
        trainer.run(args.environment, args.num_timesteps,
                    os.path.join(args.logdir, args.label))
    elif args.command == 'eval':
        trainer.eval(args.environment, args.num_timesteps,
                     os.path.join(args.logdir, args.label))
    else:
        parser.print_usage()
